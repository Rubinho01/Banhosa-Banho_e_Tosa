...
export function NovoPetForm({ tutors }: Props) {
  ...
  const [tutorName, setTutorName] = useState(tutors[0]?.name ?? '');
  ...
  async function submit(event: FormEvent) {
    ...
    setLoading(true);
    await createPetAction({ name, species, breed, size, tutorName });
    setLoading(false);
    setName('');
    setBreed('');
  ...
  return <>
    ...
    <div className="grid content-grid" style={{ gridTemplateColumns: 'minmax(0, 1.6fr) minmax(300px, .75fr)' }}>
      <form className="panel form-card" onSubmit={submit}><div className="grid form-grid">
        ...
        <div className="form-field"><label htmlFor="tutor">Tutor</label><select id="tutor" className="select" value={tutorName} onChange={(e) => setTutorName(e.target.value)}>{tutors.map((item) => <option key={item.id} value={item.name}>{item.name}</option>)}</select></div>
      </div>{message ? <div className="notice" style={{ marginTop: 16 }}>{message}</div> : null}<div style={{ marginTop: 18, display: 'flex', justifyContent: 'flex-end', gap: 10 }}><a href="/pets" className="btn btn-secondary">Cancelar</a><button className="btn btn-primary" type="submit" disabled={loading}><Icon name="check" /> {loading ? 'Salvando...' : 'Cadastrar pet'}</button></div></form>
      <aside className="summary-card"><div className="eyebrow" style={{ color: '#e2d5c4' }}>Resumo</div><h3 style={{ margin: '7px 0 12px', fontSize: 20 }}>Novo cadastro</h3><p className="muted" style={{ fontSize: 12, lineHeight: 1.5 }}>Esses dados serão usados para calcular a duração dos atendimentos com base no porte do pet.</p><div className="summary-row"><span>Nome</span><strong>{name || '—'}</strong></div><div className="summary-row"><span>Espécie</span><strong>{species}</strong></div><div className="summary-row"><span>Porte</span><strong>{size}</strong></div><div className="summary-row"><span>Tutor</span><strong>{tutorName}</strong></div></aside>
    </div>
  </>;
}
