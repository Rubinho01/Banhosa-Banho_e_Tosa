...
export default function NovoTutorPage() {
  ...
  async function submit(event: FormEvent) {
    ...
    setLoading(true);
    await createTutorAction({ name, phone, email });
    setLoading(false);
    setName('');
    setPhone('');
    setEmail('');
  ...
  return <>
    ...
    <div className="grid content-grid" style={{ gridTemplateColumns: 'minmax(0, 1.6fr) minmax(300px, .75fr)' }}>
      ...
      </div>{message ? <div className="notice" style={{ marginTop: 16 }}>{message}</div> : null}<div style={{ marginTop: 18, display: 'flex', justifyContent: 'flex-end', gap: 10 }}><a href="/tutores" className="btn btn-secondary">Cancelar</a><button className="btn btn-primary" type="submit" disabled={loading}><Icon name="check" /> {loading ? 'Salvando...' : 'Cadastrar tutor'}</button></div></form>
