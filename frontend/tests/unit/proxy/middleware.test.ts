...
function buildRequest(pathname: string, { authenticated = false } = {}) {
    ...
    const headers = authenticated ? { cookie: 'banhosa_token=eyJhbGciOiJIUzI1NiJ9.fake.token' } : undefined;
...
describe('Middleware (Proteção de Rotas)', () => {
    ...
    it('deve redirecionar para /login se o cookie do token estiver vazio (sessão corrompida/inválida)', () => {
        // 1. Arrange
        const request = new NextRequest('http://localhost:3000/tutores', {
            headers: { cookie: 'banhosa_token=' },
        });
